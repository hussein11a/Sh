...
export async function loadCmsData(contentType) {
  try {
    ...
    // محاكاة تحميل البيانات من Netlify CMS
    // في بيئة الإنتاج، يمكن استبدال هذا بطلب API حقيقي
    switch (contentType) {
      case 'site_info':
        return {
      ...
      default:
        console.warn(`نوع المحتوى غير معروف: ${contentType}`);
        return null;
    }
  } catch (error) {
    console.error(`خطأ في تحميل البيانات من Netlify CMS: ${error.message}`);
    throw error; // إعادة رمي الخطأ للتعامل معه في المستدعي
  }
}

// تفعيل Netlify Identity إذا كان موجوداً
